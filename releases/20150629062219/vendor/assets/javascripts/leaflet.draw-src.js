...
L.Draw.Polyline = L.Draw.Feature.extend({
	...
	_getTooltipText: function () {
		...
		if (this._markers.length === 0) {
			labelText = {
				text: 'Нажмите для того, чтобы начать рисовать.'
			};
		} else {
			...
			if (this._markers.length === 1) {
				labelText = {
					text: 'Нажмите чтобы продолжить рисовать линию.',
					subtext: distanceStr
				};
			} else {
				labelText = {
					text: 'Нажмите на последнюю точку, чтобы закончить рисование.',
					subtext: distanceStr
				};
			}
		}
...
L.Draw.Polygon = L.Draw.Polyline.extend({
	...
	_getTooltipText: function () {
		var text;
		if (this._markers.length === 0) {
			text = 'Для начала выделения области щелкните по карте';
		} else if (this._markers.length < 3) {
			text = 'Щелкните для продолжения';
		} else {
			text = 'Двойной щелчок для завершения выделения';
		}
		return {
			text: text
		};
...
L.EditToolbar.Edit = L.Handler.extend({
	...
	addHooks: function () {
		if (this._map) {
			...
			this._tooltip = new L.Tooltip(this._map);
			this._tooltip.updateContent({ text: 'Перемещайте указатели или точки на карте', subtext: 'Нажмите на кнопку "Отменить", для отмены изменений ' });
...
L.EditToolbar.Delete = L.Handler.extend({
	...
	addHooks: function () {
		if (this._map) {
			...
			this._tooltip = new L.Tooltip(this._map);
			this._tooltip.updateContent({ text: 'Для удаления щелкните на выделенную область ' });
